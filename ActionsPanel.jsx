...
class AmazonAdsAPI:
    ...
    def request_reports_batch(self, report_types, days=14):
        """Richiede più report in un colpo solo (Amazon li accoda ed elabora in parallelo
        lato server). Ritorna {report_type: report_id} solo per le richieste riuscite."""
        ids = {}
        for rt in report_types:
            rid = self.request_report(rt, days)
            if rid:
                ids[rt] = rid
        return ids

    def poll_reports_batch(self, report_ids, max_wait=300):
        """Aspetta il completamento di più report insieme (round-robin), invece di
        aspettare fino a max_wait per ciascuno in sequenza. Ritorna {report_type: righe}."""
        results = {}
        pending = dict(report_ids)
        if not pending:
            return results
        print(f"⏳ Attesa completamento di {len(pending)} report in parallelo...", flush=True)
        start = time.time()
        while pending and time.time() - start < max_wait:
            for rt, rid in list(pending.items()):
                try:
                    headers = self._base_headers()
                    headers["Accept"] = "application/vnd.createAsync.v3+json"
                    resp = requests.get(f"{self.base_url}/reporting/reports/{rid}", headers=headers)
                    resp.raise_for_status()
                    data = resp.json()
                    status = data.get("status", "")
                    if status == "COMPLETED":
                        ...
                        print(f"   ✅ {rt} completato ({len(results[rt])} righe)", flush=True)
                        del pending[rt]
                    elif status == "FAILURE":
                        print(f"   ❌ {rt} fallito: {data.get('failureReason', 'sconosciuto')}")
                        results[rt] = []
                        del pending[rt]
                except Exception as e:
                    print(f"   ⚠️ Errore polling {rt}: {e}", flush=True)
            if pending:
                elapsed = int(time.time() - start)
                print(f"   ... ancora in attesa: {', '.join(pending.keys())} ({elapsed}s)", flush=True)
                time.sleep(10)
        for rt in pending:
            print(f"   ⏰ Timeout attesa report {rt}", flush=True)
            results[rt] = []
        return results
...
def fetch_all_data(marketplace=None, days=14):
    ...
    report_ids = api.request_reports_batch(
        ["spCampaigns", "spKeywords", "spSearchTerm", "spTargeting", "spAdvertisedProduct"], days
    )
    reports = api.poll_reports_batch(report_ids, max_wait=300)
